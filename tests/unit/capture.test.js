...
test('losing the click watcher mid-session falls back to interval capture', () => {
  const service = makeService();
  service.settings.get = (key) => ({ 'capture.autoIntervalSec': 3, 'capture.fallbackTrigger': 'interval' })[key] ?? null;
...
test('an unset fallback preference uses the hotkey instead of a timer', () => {
  const service = makeService();
  service.settings.get = () => null;
  assert.equal(service.fallbackCaptureTrigger(), 'hotkey');
});
...
test('stored clicks automatically focus while preserving screenshot bytes and manual defaults', async () => {
  ...
  assert.deepEqual(clicked.step.focusedView, { enabled: true, zoom: 1.25, panX: 1, panY: 0 });
  ...
  assert.equal(knownWindow.step.focusedView.zoom, 1.25);
